...
def train_model():
    ...
    if st.button("Split Data"):
        ...
        with mlflow.start_run(run_name="Data_Split"):
            ...
            st.success(f"Data split completed and logged to MLflow under Experiment: '{experiment_name}'.")
    ...
    if use_cv:
        ...
        if st.button("Generate Cross Validation Folds"):
            ...
            st.success(f"Generated {k_folds}-fold cross validation and logged to MLflow under Experiment: '{experiment_name}'.")
    ...
    if st.button("Train Model"):
        ...
        with mlflow.start_run(run_name=f"{model_choice}_Titanic"):
            ...
            # Log vào MLflow với input example
            mlflow.log_params(model_params)
            mlflow.log_metric("train_accuracy", train_score)
            mlflow.log_metric("valid_accuracy", valid_score)
            # Lấy một mẫu từ X_train làm input example
            input_example = X_train.iloc[:1]
            mlflow.sklearn.log_model(model, "model", input_example=input_example)
            st.success(f"{model_choice} training completed and logged to MLflow under Experiment: '{experiment_name}'.")
