...
def preprocess_data():
    ...
    # Bắt đầu một run MLflow khi người dùng bắt đầu tiền xử lý
    with mlflow.start_run(run_name=f"Titanic_Preprocessing_{experiment_name}"):
        # Log metadata ban đầu
        mlflow.log_param("experiment_name", experiment_name)
        mlflow.log_param("initial_data_shape", st.session_state['data'].shape)
        mlflow.log_param("initial_missing_values", st.session_state['data'].isnull().sum().to_dict())
        ...
        if st.button("Loại bỏ các cột đã chọn 🗑️"):
            if columns_to_drop:
                ...
                mlflow.log_param("columns_dropped", columns_to_drop)
                mlflow.log_param("post_drop_shape", st.session_state['data'].shape)
        ...
        if missing_columns:
            for col in missing_columns:
                ...
                if st.button(f"Điền giá trị cho '{col}' ✏️", key=f"fill_{col}"):
                    ...
                    mlflow.log_param(f"{col}_fill_method", st.session_state['preprocessing_steps'][f"{col}_filled"])
                    mlflow.log_param(f"{col}_missing_count_before", st.session_state['data'][col].isnull().sum())
        else:
        ...
        if categorical_cols:
            for col in categorical_cols:
                ...
                if st.button(f"Áp dụng mã hóa cho '{col}' 🔠", key=f"apply_{col}"):
                    ...
                    mlflow.log_param(f"{col}_encoding_method", st.session_state['preprocessing_steps'][f"{col}_encoded"])
        else:
        ...
        if numerical_cols:
            ...
            if st.button("Áp dụng chuẩn hóa 📏"):
                if cols_to_scale:
                    ...
                    mlflow.log_param("scaling_method", st.session_state['preprocessing_steps']["scaling"])
                    mlflow.log_param("scaled_columns", st.session_state['preprocessing_steps'].get("scaled_columns", []))
                else:
        ...
        if st.button("Lưu dữ liệu đã xử lý và log vào MLflow 📋"):
            ...
            mlflow.log_text(st.session_state['data'].head().to_csv(), "processed_data_sample.csv")
            mlflow.log_text(pd.read_csv(uploaded_file).head().to_csv(), "original_data_sample.csv")

            # Log các tham số tiền xử lý vào MLflow
            log_preprocessing_params(st.session_state['preprocessing_steps'])
            st.success("Các bước tiền xử lý đã được log chi tiết vào MLflow! 📊")
